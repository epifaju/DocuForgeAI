...
class PackInstallationApiTest {
    ...
    static void props(DynamicPropertyRegistry registry) {
        ...
        registry.add("docuforge.storage.root", () -> "target/test-storage-pack-install");
    ...
    void setUp() throws Exception {
        ...
        companyA = saveCompany("Install Co A", "install-a");
        Company companyB = saveCompany("Install Co B", "install-b");
        saveUser(companyA, "admin@install-a.test", "AdminPass123!", RoleCode.ADMIN);
        saveUser(companyA, "viewer@install-a.test", "ViewerPass123!", RoleCode.VIEWER);
        saveUser(companyB, "admin@install-b.test", "AdminPass123!", RoleCode.ADMIN);

        adminToken = login("install-a", "admin@install-a.test", "AdminPass123!");
        viewerToken = login("install-a", "viewer@install-a.test", "ViewerPass123!");
        otherAdminToken = login("install-b", "admin@install-b.test", "AdminPass123!");
    }

    @Test
    void adminCanUploadValidateAndInstall() throws Exception {
        UUID jobId = uploadAndValidate(buildValidPackZip());
        ...
        MvcResult install = mockMvc.perform(post("/api/v1/admin/business-packs/imports/" + jobId + "/install")
                ...
                .andExpect(jsonPath("$.data.installationStatus").value("ACTIVE"))
                .andExpect(jsonPath("$.data.templatesInstalled").value(1))
                .andExpect(jsonPath("$.data.promptsInstalled").value(1))
                .andReturn();
        ...
        assertThat(packImportJobRepository.findById(jobId).orElseThrow().getStatus())
                .isEqualTo(PackImportJobStatus.INSTALLED);
        ...
        assertThat(template.getOrigin()).isEqualTo(TemplateOrigin.PACK);
        assertThat(template.getStatus()).isEqualTo(TemplateStatus.ACTIVE);
        assertThat(template.getCurrentVersion()).isNotNull();
        assertThat(templateVariableRepository.findByTemplateVersionIdOrderByDisplayOrderAsc(
                template.getCurrentVersion().getId())).hasSize(1);
        assertThat(businessPackPromptRepository.findAll()).hasSize(1);
        assertThat(businessPackFileRepository.findAll()).isNotEmpty();
        assertThat(installationRepository.findAll()).hasSize(1)
                .first()
                .extracting(i -> i.getInstallationType(), i -> i.getStatus())
                .containsExactly(PackInstallationType.FRESH, PackInstallationStatus.ACTIVE);

        assertThat(auditLogRepository.findAll().stream().map(a -> a.getAction()).toList())
                .contains(AuditActions.PACK_INSTALLED);
    ...
    @Test
    void installRequiresValidStatusAndAdminRole() throws Exception {
    ...
    @Test
    void templateCodeCollisionRollsBackInstallation() throws Exception {
        Template existing = new Template();
        existing.setCompany(companyA);
        existing.setCode("DEMO_QUOTE");
        existing.setName("User Template");
        existing.setOrigin(TemplateOrigin.USER);
        ...
        assertThat(packImportJobRepository.findById(jobId).orElseThrow().getStatus())
                .isEqualTo(PackImportJobStatus.FAILED);
        assertThat(businessPackRepository.findAll()).isEmpty();
        assertThat(businessPackVersionRepository.findAll()).isEmpty();
        assertThat(templateRepository.findAll()).hasSize(1);
        assertThat(auditLogRepository.findAll().stream().map(a -> a.getAction()).toList())
                .contains(AuditActions.PACK_INSTALLATION_FAILED);
    }
        ...
        String manifest = """
                {
                  ...
                  "version": "1.0.0",
                  "type": "CUSTOM",
                  "description": "Install API fixture",
