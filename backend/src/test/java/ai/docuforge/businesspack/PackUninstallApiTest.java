...
/**
 * Soft uninstall — PRD §§94–96, §151.
 */
@SpringBootTest
@AutoConfigureMockMvc
@Testcontainers
class PackUninstallApiTest {
    ...
    static void props(DynamicPropertyRegistry registry) {
        ...
        registry.add("docuforge.storage.root", () -> "target/test-storage-pack-uninstall");
    ...
    void setUp() throws Exception {
        ...
        companyA = saveCompany("Uninstall Co A", "uninst-a");
        Company companyB = saveCompany("Uninstall Co B", "uninst-b");
        saveUser(companyA, "admin@uninst-a.test", "AdminPass123!", RoleCode.ADMIN);
        saveUser(companyA, "viewer@uninst-a.test", "ViewerPass123!", RoleCode.VIEWER);
        saveUser(companyB, "admin@uninst-b.test", "AdminPass123!", RoleCode.ADMIN);

        adminToken = login("uninst-a", "admin@uninst-a.test", "AdminPass123!");
        viewerToken = login("uninst-a", "viewer@uninst-a.test", "ViewerPass123!");
        otherAdminToken = login("uninst-b", "admin@uninst-b.test", "AdminPass123!");
    }

    @Test
    void uninstallWithHistoricalDocumentsKeepsMetadataAndBlocksNewGeneration() throws Exception {
        ...
        UUID versionId = businessPackRepository.findById(packId).orElseThrow().getCurrentVersion().getId();

        UUID docId = generateDocument(templateId, "Historical Quote", "Alice");

        mockMvc.perform(delete("/api/v1/admin/business-packs/" + packId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.packId").value(packId.toString()))
                .andExpect(jsonPath("$.data.status").value("UNINSTALLED"))
                .andExpect(jsonPath("$.data.templatesArchived").value(1))
                .andExpect(jsonPath("$.data.historicalDocumentCount").value(1))
                .andExpect(jsonPath("$.data.metadataPreserved").value(true));
        ...
        assertThat(businessPackVersionRepository.findById(versionId)).isPresent();
        assertThat(templateRepository.findById(templateId).orElseThrow().getStatus())
                .isEqualTo(TemplateStatus.ARCHIVED);
        assertThat(installationRepository.findAll()).hasSize(1)
                .first()
                .satisfies(i -> {
                    assertThat(i.getStatus()).isEqualTo(PackInstallationStatus.UNINSTALLED);
                    assertThat(i.getUninstalledAt()).isNotNull();
                });
        ...
        mockMvc.perform(post("/api/v1/admin/business-packs/" + packId + "/enable")
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isConflict());

        assertThat(auditLogRepository.findAll().stream().map(a -> a.getAction()).toList())
                .contains(AuditActions.PACK_UNINSTALLED);
    }

    @Test
    void uninstallWithoutDocumentsIsSoftAndIdempotent() throws Exception {
        UUID packId = installDemoPack();
        long versionCountBefore = businessPackVersionRepository.count();
        long templateCountBefore = templateRepository.count();
        long fileCountBefore = businessPackFileRepository.count();

        mockMvc.perform(delete("/api/v1/admin/business-packs/" + packId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("UNINSTALLED"))
                .andExpect(jsonPath("$.data.historicalDocumentCount").value(0))
                .andExpect(jsonPath("$.data.metadataPreserved").value(true));

        assertThat(businessPackVersionRepository.count()).isEqualTo(versionCountBefore);
        assertThat(templateRepository.count()).isEqualTo(templateCountBefore);
        assertThat(businessPackFileRepository.count()).isEqualTo(fileCountBefore);
    ...
    @Test
    void uninstallForbiddenForViewerAndCrossTenant() throws Exception {
        UUID packId = installDemoPack();
    ...
    private UUID generateDocument(UUID templateId, String title, String clientName) throws Exception {
        MvcResult generated = mockMvc.perform(post("/api/v1/documents/generate")
                        ...
                        .content("""
                                ...
                                """.formatted(templateId, title, clientName)))
                .andExpect(status().isOk())
                .andReturn();
        return UUID.fromString(objectMapper.readTree(generated.getResponse().getContentAsString())
        ...
        String manifest = """
                {
                  ...
                  "version": "1.0.0",
                  "type": "CUSTOM",
                  "description": "Uninstall API fixture",
