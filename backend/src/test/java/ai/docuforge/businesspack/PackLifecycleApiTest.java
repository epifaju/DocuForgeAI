...
class PackLifecycleApiTest {
    ...
    static void props(DynamicPropertyRegistry registry) {
        ...
        registry.add("docuforge.storage.root", () -> "target/test-storage-pack-lifecycle");
    ...
    void setUp() throws Exception {
        ...
        companyA = saveCompany("Life Co A", "life-a");
        Company companyB = saveCompany("Life Co B", "life-b");
        saveUser(companyA, "admin@life-a.test", "AdminPass123!", RoleCode.ADMIN);
        saveUser(companyA, "viewer@life-a.test", "ViewerPass123!", RoleCode.VIEWER);
        saveUser(companyB, "admin@life-b.test", "AdminPass123!", RoleCode.ADMIN);

        adminToken = login("life-a", "admin@life-a.test", "AdminPass123!");
        viewerToken = login("life-a", "viewer@life-a.test", "ViewerPass123!");
        otherAdminToken = login("life-b", "admin@life-b.test", "AdminPass123!");
    }

    @Test
    void disablePackBlocksNewGenerationButKeepsHistoricalDocument() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/admin/business-packs/" + packId + "/disable")
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("DISABLED"));

        assertThat(businessPackRepository.findById(packId).orElseThrow().getStatus())
                .isEqualTo(BusinessPackStatus.DISABLED);
        ...
        assertThat(installationRepository.findAll()).hasSize(1)
                .first()
                .satisfies(i -> {
                    assertThat(i.getStatus()).isEqualTo(PackInstallationStatus.DISABLED);
                    assertThat(i.getDisabledAt()).isNotNull();
                });
        ...
        assertThat(auditLogRepository.findAll().stream().map(a -> a.getAction()).toList())
                .contains(AuditActions.PACK_DISABLED);
        ...
        assertThat(auditLogRepository.findAll().stream().map(a -> a.getAction()).toList())
                .contains(AuditActions.PACK_ENABLED);
    }

    @Test
    void templateDisableEnablePreservesIndividualChoiceAcrossPackToggle() throws Exception {
        UUID packId = installDemoPack();

        mockMvc.perform(post("/api/v1/admin/business-packs/" + packId + "/templates/DEMO_QUOTE/disable")
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.enabled").value(false));

        assertThat(templateRepository.findByCompanyIdAndCode(companyA.getId(), "DEMO_QUOTE").orElseThrow().getStatus())
                .isEqualTo(TemplateStatus.DRAFT);
        assertThat(auditLogRepository.findAll().stream().map(a -> a.getAction()).toList())
                .contains(AuditActions.PACK_TEMPLATE_DISABLED);
        ...
        // Individually disabled template must stay DRAFT after pack re-enable
        assertThat(templateRepository.findByCompanyIdAndCode(companyA.getId(), "DEMO_QUOTE").orElseThrow().getStatus())
                .isEqualTo(TemplateStatus.DRAFT);

        mockMvc.perform(post("/api/v1/admin/business-packs/" + packId + "/templates/DEMO_QUOTE/enable")
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.enabled").value(true));

        assertThat(auditLogRepository.findAll().stream().map(a -> a.getAction()).toList())
                .contains(AuditActions.PACK_TEMPLATE_ENABLED);
    }

    @Test
    void cannotEnableTemplateWhilePackDisabledAndViewerForbidden() throws Exception {
        UUID packId = installDemoPack();
