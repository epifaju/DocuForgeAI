...
class DocumentVersioningTest {
    ...
    static void props(DynamicPropertyRegistry registry) {
        ...
        registry.add("docuforge.storage.root", () -> "target/test-storage-versioning");
    ...
    void setUp() throws Exception {
        ...
        company.setName("Ver Co");
        company.setIdentifier("ver-co");
        company = companyRepository.save(company);

        UserAccount admin = new UserAccount();
        admin.setCompany(company);
        admin.setEmail("admin@ver-co.test");
        ...
        adminToken = login("ver-co", "admin@ver-co.test", "AdminPass123!");
        templateId = createActiveTemplate();
    }

    @Test
    void newVersionCreatesSiblingWithoutMutatingSource() throws Exception {
        UUID sourceId = generateDocument("Contrat v1", "Alice");
        GeneratedDocument sourceBefore = generatedDocumentRepository.findById(sourceId).orElseThrow();
        String sourceChecksum = sourceBefore.getChecksum();
        String sourceRef = sourceBefore.getReference();
        DocumentStatus sourceStatus = sourceBefore.getStatus();

        MvcResult created = mockMvc.perform(post("/api/v1/documents/" + sourceId + "/new-version")
                ...
                .andExpect(jsonPath("$.data.documentVersionNumber").value(2))
                .andExpect(jsonPath("$.data.parentDocumentId").value(sourceId.toString()))
                .andExpect(jsonPath("$.data.rootDocumentId").value(sourceId.toString()))
                .andExpect(jsonPath("$.data.title").value("Contrat v2"))
                .andReturn();
        ...
        GeneratedDocument sourceAfter = generatedDocumentRepository.findById(sourceId).orElseThrow();
        assertThat(sourceAfter.getChecksum()).isEqualTo(sourceChecksum);
        assertThat(sourceAfter.getReference()).isEqualTo(sourceRef);
        assertThat(sourceAfter.getStatus()).isEqualTo(sourceStatus);
        assertThat(sourceAfter.getDocumentVersionNumber()).isEqualTo(1);

        mockMvc.perform(get("/api/v1/documents/" + sourceId + "/versions")
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.length()").value(2))
                .andExpect(jsonPath("$.data[0].documentVersionNumber").value(1))
                .andExpect(jsonPath("$.data[1].documentVersionNumber").value(2))
                .andExpect(jsonPath("$.data[1].id").value(newId.toString()));

        assertThat(generatedDocumentRepository.count()).isEqualTo(2);
    }

    @Test
    void getIncludesDataSnapshotForPrefill() throws Exception {
        UUID sourceId = generateDocument("Snap", "Alice");
        mockMvc.perform(get("/api/v1/documents/" + sourceId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.data['client.firstName']").value("Alice"))
                .andExpect(jsonPath("$.data.documentVersionNumber").value(1));
    }

    private UUID generateDocument(String title, String firstName) throws Exception {
        MvcResult generated = mockMvc.perform(post("/api/v1/documents/generate")
                        ...
                        .content("""
                                ...
                                """.formatted(templateId, title, firstName)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.documentVersionNumber").value(1))
                .andReturn();
    ...
    private UUID createActiveTemplate() throws Exception {
        MvcResult created = mockMvc.perform(post("/api/v1/templates")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"code":"ver_tpl","name":"Version template"}
                                """))
                .andExpect(status().isCreated())
                .andReturn();
